...
- **Settings**: every hook in `settings.json` points at an existing file; baseline permission entries present; the `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` env cap present and `"2"`.
