...
7. **Git hygiene** — `.gitignore` covers: secrets patterns (`.env*`, keys), large binary or vendored assets (the project's classes per `work/PROJECT.md`), `tmp/`, `.codebase-memory/` (the local graph index — BASELINE index policy); `git status` is not carrying obviously-unintended files.
