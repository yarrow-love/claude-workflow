...
3. **The hand-off phase** (when an external design tool is in play) — file `work/design/sessions/<date>.<slug>/BRIEF.md`; dispatch the **inspector** to capture `current-state/` (png+html per relevant surface); snapshot the system assets as-sent. (No external tool → skip; the brief goes straight to operator sign-off.)
