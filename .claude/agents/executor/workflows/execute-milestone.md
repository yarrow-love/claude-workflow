...
3.  **Reconnaissance.** Dispatch one researcher subagent per phase, in parallel (read-only). Brief each to read its phase doc + the codebase areas it touches + the relevant `work/footguns/` ledger files (see the CLAUDE.md index) (+ Context7 per the `work/PROJECT.md` library list), and to return: underspecifications, architectural risks, ordering hazards, cross-phase dependencies, and decisions that should be made before building. This research feeds forward into each phase's planning — it is not throwaway.
...
8.  **Objective close.** When all phases reach acceptance (in-repo certified + operator-gated handed off): update the milestone `MILESTONE.md` + roadmap row (status), reconcile the footgun ledger (`work/footguns/` + the CLAUDE.md index), write the campaign **Summary** and the consolidated **Operator Handoff**, and move the campaign log to `work/todos/_done/`.
