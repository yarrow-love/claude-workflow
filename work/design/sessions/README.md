...
| everything else at the root | in | The returns: `index.html`, assets, `components/<c>.html+.png` comps — pulled via the DesignSync harness tool (`get_file`, ≤256 KiB each; oversized binaries operator-exported), or by manual carry on the degrade path |
