...
- **Ten commands, two of them polymorphic** — `/plan` and `/execute` route by the argument's tier (mission / milestone / work order); `/design` runs the judgment-closed design dialogs (canon, hand-off briefs, taste gates); plus `/triage`, `/investigate`, `/research`, `/document`, `/close`, `/archive`, `/initialize`.
